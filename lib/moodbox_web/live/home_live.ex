  ...
  def render(assigns) do
    ...
    <div class="relative overflow-hidden h-screen w-full">
      <div class="relative h-full z-10 px-4 py-8 sm:px-24 sm:py-20 xl:py-32">
        <h1
          phx-mounted={JS.transition({"ease-in duration-700 delay-100", "opacity-0", "opacity-100"})}
        ...
        <h2
          phx-mounted={JS.transition({"ease-in duration-700 delay-1000", "opacity-0", "opacity-100"})}
        ...
        <.link patch={~p"/moods"}>
          <button
            class="px-8 py-4 xl:py-6 xl:px-12 xl:text-xl font-bold tracking-wider cursor-pointer rounded-full bg-[#6b2a6d] text-white"
            phx-mounted={
              JS.transition(
                {"ease-in duration-700 delay-1000", "opacity-0",
                 "opacity-100 transition hover:scale-105"}
              )
            }
          >
